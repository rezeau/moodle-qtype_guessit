...
Feature: Test all the basic functionality of this guessit (wordle) question type
  ...
  Scenario: Create, edit and preview a wordle question
    ...
    And I set the field with xpath "//input[contains(@id, '1_p2')]" to "a"
    And I set the field with xpath "//input[contains(@id, '1_p3')]" to "s"
    And I set the field with xpath "//input[contains(@id, '1_p4')]" to "t"
    ...
    And I set the field with xpath "//input[contains(@id, '1_p3')]" to "s"
    And I set the field with xpath "//input[contains(@id, '1_p4')]" to "t"
    And I set the field with xpath "//input[contains(@id, '1_p5')]" to ""
    And I press "Check"
    Then I should see "Please enter an answer in ALL the gaps."
    And I press "Start again"
